...
import { mapCreditToCanonical } from '../mappers/creditMapper';
import { withResilience } from '../resilience';
import { logger } from '../telemetry/logger';
import { traceAsync } from '../telemetry/spans';
import { context, propagation } from '@opentelemetry/api';

export class CreditAdapter implements AccountAdapter {
  accountType = 'creditcard' as const;
  backendName = 'credit-service-v1';
  private baseUrl: string;
  private apiKey: string;

  constructor(baseUrl?: string, apiKey?: string) {
    this.baseUrl = baseUrl || process.env.CREDIT_SERVICE_URL || 'http://localhost:3002';
    this.apiKey = apiKey || process.env.CREDIT_API_KEY || 'credit-demo-key-456';
  }
    ...
    return traceAsync(
      ...
      async (span) => {
        const exec = async () => {
          ...
          span.setAttribute('http.url', `${this.baseUrl}/cards/${accountId}`);
          span.setAttribute('http.method', 'GET');

          const response = await fetch(`${this.baseUrl}/cards/${accountId}`, {
            headers,
          ...
          return mapCreditToCanonical(raw);
        };
        ...
        try {
          ...
          logger.debug('Credit adapter success', {
            accountId,
            latencyMs: latency,
          });
          ...
          logger.warn('Credit adapter error', {
            accountId,
            error: err.message,
            latencyMs: latency,
          });
