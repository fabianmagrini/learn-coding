...
export class BankAdapter implements AccountAdapter {
  ...
  constructor(baseUrl?: string, apiKey?: string) {
    this.baseUrl = baseUrl || process.env.BANK_SERVICE_URL || 'http://localhost:3001';
    this.apiKey = apiKey || process.env.BANK_API_KEY || 'bank-demo-key-123';
  }
    ...
    return traceAsync(
      ...
      async (span) => {
        const exec = async () => {
          ...
          span.setAttribute('http.url', `${this.baseUrl}/accounts/${accountId}`);
        ...
        try {
          ...
          logger.debug('Bank adapter success', {
            accountId,
            latencyMs: latency,
          });
          ...
          logger.warn('Bank adapter error', {
            accountId,
            error: err.message,
            latencyMs: latency,
          });
