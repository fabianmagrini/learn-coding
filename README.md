...
- **Features**: Case-sensitive and case-insensitive line search (via a `-i`/`--ignore-case` flag or the `IGNORE_CASE` env var), input from a file, a directory (searched recursively for `.txt` files with `grep -r`-style file-name prefixes), or piped via standard input, optional 1-based line numbers (`-n`/`--line-number`), ANSI bold-red highlighting of matches when writing to a terminal (auto-disabled when piped), auto-generated `--help`/`--version`, friendly error handling with clean exit codes, and a suite of unit tests; designed as a hands-on introduction to Rust's core concepts.
