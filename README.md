...
- **Features**: Case-sensitive and case-insensitive line search (via a `-i`/`--ignore-case` flag or the `IGNORE_CASE` env var), input from a file, a directory (searched recursively with `grep -r`-style file-name prefixes and repeatable `--include=GLOB`/`--exclude=GLOB` filters, include defaulting to `*.txt`), or piped via standard input, optional 1-based line numbers (`-n`/`--line-number`), a match-count mode (`-c`/`--count`, per file in a directory), `grep`-style ANSI colourised output (bold-red matches, magenta file names, green line numbers) with a `--color=auto|always|never` override (auto-disabled when piped), auto-generated `--help`/`--version`, friendly error handling with clean exit codes, and a suite of unit tests; designed as a hands-on introduction to Rust's core concepts.
- **Technology Stack**: Rust, Cargo, clap (derive-based CLI parsing), glob (file-name pattern matching)
- **Architecture**: Standard Rust CLI layout with a thin binary (`src/main.rs`) over a testable library (`src/lib.rs`), demonstrating ownership and borrowing (lifetime-annotated string slices), `Result`/`?` error propagation, a `clap::Parser`-derived config struct, `Option`/`match` for file-vs-stdin input, recursive directory traversal with `fs::read_dir` and `Path`, glob-based file filtering, iterators and closures (`enumerate` for line numbers), tuple destructuring, a `ValueEnum` for `--color`, TTY-aware ANSI colour output via `IsTerminal`, and built-in `#[cfg(test)]` unit testing.
